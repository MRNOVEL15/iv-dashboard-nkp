...
# ดึงข้อมูลจาก Google Sheets ทุกๆ 30 วินาที
...
def load_data():
    try:
        ...
        # --- ฐานข้อมูลข้อควรระวัง (Precautions & Recommendations) ---
        drug_precautions = {
...
st.markdown("""
    <div class="header-banner">
        ...
        <p>ศูนย์ข้อมูลยา (DIS) • โรงพยาบาลนครพิงค์</p>
    </div>
""", unsafe_allow_html=True)
...
if df is not None:
    ...
    # ------------------------------------
    # ฝั่งขวา: คู่ยาที่ใช้บ่อย (Quick Links)
    # ------------------------------------
    with col_side:
    ...
    # ------------------------------------
    # ฝั่งซ้าย: ค้นหาและแสดงผล (Search & Results)
    # ------------------------------------
    with col_main:
        st.markdown('<div class="panel-box">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">🔍 ตรวจสอบความเข้ากันได้ของยาฉีด (Admixture & Y-Site)</div>', unsafe_allow_html=True)
        
        idx_a = df.index.tolist().index(st.session_state.drug_a) if st.session_state.drug_a in df.index else 0
        idx_b = df.columns.tolist().index(st.session_state.drug_b) if st.session_state.drug_b in df.columns else 0
        
        c1, c2 = st.columns(2)
        with c1: drug_a = st.selectbox("💉 ยาตัวที่ 1 (สารละลายหลัก / ยาหลัก)", df.index.tolist(), index=idx_a)
        with c2: drug_b = st.selectbox("💉 ยาตัวที่ 2 (ยาที่ให้ร่วม / ยาให้ผ่าน Y-Site)", df.columns.tolist(), index=idx_b)
